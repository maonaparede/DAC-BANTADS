...
export class RegisterComponent {
    ...
    if (this.registerForm.valid)
      try {
        const user: IUserAutocadastro = {
          ...
          cpf: this.removeNonNumericCharacters(this.registerForm.value.cpf) || '',
          telefone: this.removeNonNumericCharacters(this.registerForm.value.phone) || '',
          salario:  Number(this.removeNonNumericCharacters(this.registerForm.value.salary)) || 0,
          cep: this.registerForm.value.cep || '',
          numero: Number(this.removeNonNumericCharacters(this.registerForm.value.number)) || 0,
          estado: this.state || '',
          cidade: this.city || '',
  ...
  private removeNonNumericCharacters(str : any) : string {
    const texto = String(str).replace(/\D/g, ''); 
    return texto;
  }
